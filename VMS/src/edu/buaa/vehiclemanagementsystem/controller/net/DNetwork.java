 ...
 */
public class DNetwork implements Network {
	 ...
	 */
	public DNetwork(HttpStack httpStack) {
	 ...
	 */
	public DNetwork(HttpStack httpStack, ByteArrayPool pool) {
			...
			try {
				...
				if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
					return new NetworkResponse(HttpStatus.SC_NOT_MODIFIED,
							request.getCacheEntry() == null ? null
	...
	private static Map<String, String> convertHeaders(Header[] headers) {
		...
		for (int i = 0; i < headers.length; i++) {
			...
			if (!result.containsKey(name)) {
				result.put(name, headers[i].getValue());
			} else {
				String string = result.get(name);
				result.put(headers[i].getName(), string + ";" + headers[i].getValue());
			}
		}
		return result;
	}
